...
> **주의:** `CLAUDE.md`는 `AGENTS.md`를 가리키는 심볼릭 링크다. 별도 사본이 아니므로 한쪽만 따로 편집하지 말 것. 새 에이전트 지원을 추가할 때도 사본을 만들지 말고 링크만 건다(예: `ln -s AGENTS.md GEMINI.md`). 클론 후 링크가 일반 텍스트 파일(`AGENTS.md` 한 줄)로 풀려 보이면 `git config core.symlinks true` 후 재체크아웃한다.
