...
function HomeView({
  ...
  error: string | null;
  runRecommend: (input: RecommendInput) => void;
  setView: (v: View) => void;
}) {
...
function ResultView({
  ...
  // 공유 toast와 "복사됨" 피드백 모두 동일한 auto-dismiss 패턴(ref + clearTimeout + 언마운트 정리)을 사용한다.
    ...
    <div className="max-w-3xl mx-auto bg-white min-h-screen border-x border-slate-100 pb-10">
      ...
      <div className="p-4 sm:p-6">
        ...
        <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {/* 키보드 접근성: 슬라이더 role + 좌우 화살표로 0.5점 단위 조정 */}
          <div
            className="flex justify-center gap-2 mt-4 outline-none rounded-lg focus-visible:ring-2 focus-visible:ring-blue-400"
            role="slider"
            tabIndex={0}
            aria-label="추천 결과 별점"
            aria-valuemin={0}
            ...
            onKeyDown={(e) => {
              if (e.key === 'ArrowRight' || e.key === 'ArrowUp') {
                e.preventDefault();
                setRating((r) => Math.min(5, r + 0.5));
              } else if (e.key === 'ArrowLeft' || e.key === 'ArrowDown') {
                e.preventDefault();
                setRating((r) => Math.max(0, r - 0.5));
              }
            }}
          >
            {[1, 2, 3, 4, 5].map((n) => {
              ...
              return (
                <div
                  key={n}
                  className="relative w-[34px] h-[34px] cursor-pointer"
                  onMouseMove={(e) =>
                    setHoverRating(e.nativeEvent.offsetX < 17 ? n - 0.5 : n)
                  }
                  onClick={(e) => setRating(e.nativeEvent.offsetX < 17 ? n - 0.5 : n)}
                >
                  <Star size={34} className="text-slate-300 pointer-events-none" fill="none" />
                  <div
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      ...
      {view === 'home' && (
        <HomeView error={error} runRecommend={runRecommend} setView={setView} />
      )}
      {view === 'step' && (
        <StepByStepView error={error} runRecommend={runRecommend} setView={setView} />
      )}
      {view === 'results' && <ResultView result={result} setView={setView} />}
    </div>
  );
}
