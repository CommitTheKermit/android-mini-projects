...
`"조용한 사무용"`처럼 **"조용한"을 직접 말하면** LLM이 그걸 명시 소프트 `조용함`으로 뽑고,
명시 우선 규칙에 따라 사무용의 `저소음=필수`가 **차단되어 소프트로 약화**됩니다.
의도만 줬을 때(`사무용`)는 `저소음`이 하드로 걸립니다. 이는 합의된 "명시 우선" 동작의 결과이며,
필요하면 별도 조정(예: 소음 차원은 명시가 더 강한 하드일 때만 의도를 막기) 가능합니다.
